### **_Знакомство с_** **_Cinema_** **_4_****_D_**
...
### **_Глава 1. Интерфейс, функции и настройки_**
...
### **_Глава 2. Параметрическое моделирование_**
...
- **_Bevel_** **_+_****_Correction_** это создание фаски с коррекцией углов у объектов с несоединённой крышкой / с возможностью создания разной фаски в разных местах
...
### **_Глава 5. Материалы и текстуры_**
...
### **_Каналы Diffusion and Luminance_**
...
### **_Урок 5. Продолжаем канал Reflectance_**
...
### **_Урок 6. Anisantropy. Текстурирование металла_**
...
### **_Урок 8. Environment+Fog+Normal+Displacement_**
...
### **_Урок 10. SubSurfaceScattering_**
...
### **_Глава 8. Анимации в Cinema 4D_**
...
**_Создание бесконечной анимации из шестиугольников_**
...
- Накидываем на плэйн тэг содуарения
...
- И тут мы меняем разными масками на одном слое оттенки изображения с помощью хуешки
...
### **_Глава 10. Продвинутое моделирование (полигональное)_**
...
- **_Loop_** **_selection_** **_(_****_U_****_+_****_L_** **_- шорткат)_** позволяет выбрать кольцо элементов
...
### **_Практическая часть_**

- Нажимаем F4. На shift+V открываем настройки вьюпорта и в back поместить нашу картинку. Подогнать примерный размер и в данном случае уменьшить до 500 и немного уменьшить яркость схемы, по которой будем чертить

- Создаём объект полигон и меняем ориентацию, а дальше подгоняем размер по корпусу (нужно, чтобы размер полигона был ровно в 10 раз больше оригинального, чтобы было удобно работать с объектом, а затем можно размеры уменьшить) (это делает просто для примера, чтобы примерно сметать будущие размеры объекта)
...
Далее вызываем **_Polygon_** **_Pen_** (M-E)

- Далее в режиме изменения граней с зажатым ctrl вытягиваем грани круга один за другим и за точки поправляем

- Если бриблизить изображение, то автоматически будет выбираться вместо грани площадь и зажав ctrl можно будет удалить либо грань либо тыкнуть по белому выделению грань

- И дальше после удаления грани соединить существующие

- Чтобы линии шли точнее по линии, можно немного точки граней вытягивать выше

-

-

- Далее, чтобы подправить расположение линий нужно воспользоваться данным инструментом (предварительно в режиме прошлого инструмента выделив область сплайна, с которой будем работать(два раза тыкнув по одному полигону))

- Выбираем такие настройки и обычными движениями поправляем наш сплайн

- Далее отключаем сабдив, тыкаем по примитиву, выбираем path selection и выделяем контур

- Далее нужно перевести выделение в сплайн

- После нажатия по нему появляется сплайн наших полигонов

- Копируем сплайн и скрываем прошлый объект

- далее нужно выпрямить наш сплайн и есть два способа:

1. Сложный, где нужно конвертнуть сплайны в джоинтсы и с инструментом rotation выпрямлять их и обратно восстанавливать из джоинтов в сплайн

2. Простой (на самом деле просто быстрый). Переносим ноль объект в конечную точку сплайна, накидываем на сплайн spline dynamics и уже можно проверить будет ли падать сплайн вниз. Падает? Отлично. Далее накидываем тег constraint. И в режиме выделения граней выбираем выделение работу с точками, выделяем первую точку нашего сплайна, закидываем нуль в object и уже с выделенной точкой на сплайне тыкаем на Set. Тыкаем на пробел и дожидаемся, пока сплайн не успокоится и поставим на паузу воспроизведение. Удаляем тэги со сплайна. Располагаем якорную точку в конечной точке сплайна в режиме работы с объектами (для этого в режиме работы с осями тыкнем по move инструменту и кликнем по точке нашего сплайна, чтобы туда телепортировать точку). Повернём на 90 градусов сплайн и сожмём по верхней оси, чтобы сплайн стал идеально-ровным

- Чтобы точно уменьшить размер под нужный тебе, нужно выбрать scale, тыкнуть по магнитику (snapping), в дынном случае зажать ctrl и тыкнуть правой кнопкой мыши удерживая повести вправо до самой последней точки и затем уже отпустить ctrl и с правой кнопкой мыши довести до нужного размера

-

- Перевели экструд в полигональный и закинули FFD

- Если не получится перевести экструд в полигоналку, то можно просто сгруппировать в null и экструд и деформер

- И с помощью FFD выделяем точки и scale-ом меняем изгиб чёрной линии по чертежу

- И методично выбираем rectangle selection tool и приступаем выделять им нужные точки и с помощью скейла их менять, тем самым создавая нужное выдавливание в объекте

-

- И уже в конечном объекте выделяем неправильные точки и подгоняем под чертёж

- Кидаем в нашу длинную вырезанную модель деформер и деформируем по сплайну из самого начала (который делали по диску)

- После подгонки делаем current state to object
