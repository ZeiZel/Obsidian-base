...
Что мы можем менять через дескриптор? Мы можем задать модификаторы для метода: его конфигурируемость, читабелность, поменять значения, задать гет/сет
...
На примере можно увидеть, что теперь вместо ==ошибки== функция выдаёт ==no error==
...
И тут сразу хочется отметить. что если мы оставим асинк/авейты, то у нас в выводе будет фигурировать промис
...
Так же мы можем через defineProperty поменять [дескрипторы](../../FrontEnd%20-%20JavaScript,%20React,%20Redux,%20NextJS/Полный%20курс%20по%20JavaScript%20+%20React%20-%20с%20нуля%20до%20результата%20(2022)/_lessons/03%20-%20Дополнительные%20основы%20JavaScript.md#006%20д%20Дескрипторы%20свойств%20и%20полезные%20методы%20объектов) для свойств и заменить стоковые методы (гет/сет) и модификаторы (читабельность, изменяемость и так далее) для этих свойств
