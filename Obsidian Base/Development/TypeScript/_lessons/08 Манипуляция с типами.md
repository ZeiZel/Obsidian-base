...
console.log(res);
```
![](_png/Pasted%20image%2020220924120754.png)
...
![](_png/Pasted%20image%2020220921193208.png)
...
Кондишенл условия нам позволяют в одну строку записать какие-то короткие для выполнения условия
Кондишенл же в типах позволяет нам определить какой тип будет у объекта при определённых вводных. Конкретно в примере, data будет иметь тип строки, если в дженерик передали 'success' 
...
Тут уже приведён пример использования кондишенл типов для проверки возвращаемого типа из тайпа. Данный тайп содержит в себе дженерик, который при расширении от числа будет возвращать User, а в остальных случаях - UserPersistend
Однако нам нужно обязательно будет скастовать ретёрны к нашему возвращаемому типу, так как тайпы не работают в рантайме и выдают ошибку
...
Чтобы всё заработало, нужно явно типизировать нашу транзакцию
Так:
```TS
...
Так же через вилку можно добавить ещё значения для исключения из мапы

![](_png/Pasted%20image%2020220923194650.png)
...
Самый простой и частый пример использования таких темплейтов - это задание шаблонов значения для разных типов ответов (например, нам нужно реализовать еррор в http и для другой структуры)

Или на том же фронте отделить ==fade-In== и ==fade-Out== через такую конструкцию

![](_png/Pasted%20image%2020220923200904.png)

Так же мы можем <font style="color: red; font-weight: bold">распаковать наш составной тип</font> и работать с его изначальным значением. 
