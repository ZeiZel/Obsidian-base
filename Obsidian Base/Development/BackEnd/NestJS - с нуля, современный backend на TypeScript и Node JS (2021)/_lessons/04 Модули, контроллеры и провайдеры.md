...
У всех приложений на NestJS должен быть хотя бы один модуль (`AppModule`)
...
- `controllers: []` - контроллеры данного модуля (позволяют с ним связаться извне - http-контроллеры запросов)
...
Часто, при подключении определённых модулей (например БД), нам нужно не просто импортировать определённый модуль, но и так же нужно передать определённую информацию (например, строку подключения к базе данных). Обычно для таких вещей подключают динамические модули. 
...
Это стандартная запись провайдера класса. Она автоматически сокращена до записи одного массива с наименованием класса.
...
Тут мы так же можем провайдить инстанс класса и использовать вместо класса величину. 
...
Так же мы можем использовать `useValue` не с классом.

![](_png/Pasted%20image%2020230203165203.png)

Провайдер `useFactory` позволяет нам инициализировать класс с уже готовыми конфигами (возвращаем новый инстанс класса с данными параметрами). Удобнее всего использовать для инициализации базы данных с некоторыми настройками.
...
В NestJS есть несколько областей:
- ==default== - пустые параметры `@Injectable()`. На всё приложение будет 1 инстанс данного сервиса. Это приводит к использованию паттерна ==Singleton==.
...
![](_png/Pasted%20image%2020230203170401.png)
