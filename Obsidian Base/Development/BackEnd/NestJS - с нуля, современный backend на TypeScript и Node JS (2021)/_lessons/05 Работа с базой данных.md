...
Ну и так же далее нам нужно сделать *timeStamp*, который будет помечать время создания объекта. Можно создать его отдельным свойством (`createdAt`), а можно сделать правильно и расширить ДТОшку от тайпгуза через `extends TimeStamp`

![](_png/Pasted%20image%2020230402162836.png)

Вместе с этим нужно добавить ещё и `_id` в нашу модель. Это можно сделать ещё более красивым способом - заэкстендить интерфейс с именем класса модели данных от класса `Base`. И тут нужно будет сказать, что в наши модели данных теперь нельзя добавлять поле `_id`, так как оно присутствует в `Base`
