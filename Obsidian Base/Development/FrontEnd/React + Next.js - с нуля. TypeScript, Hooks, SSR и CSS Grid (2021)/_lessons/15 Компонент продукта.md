...
И теперь вместо слова ***фичи*** воткнём вывод характеристик продукта. Для этого мы в `<span>` закине 
...
Тут нужно сказать, что мы накидываем сетку на каждый элемент списка отдельно
