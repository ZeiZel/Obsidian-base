...
- `ARG` - аргументы - это дополнительные параметры, которые можно передать при сборке. Можно передать как заранее определённую переменную со своим значеннием, так и неопределённую, значение которой мы передадим из вне внутри команды `docker build`. Второй вариант нужен, когда нам нужно, чтобы значение существовало только в рамках билда, но не попало на прод 
...
- `ADD` - добавляет файлы с хостовой машины в образ
- `COPY` - просто копирует файлы в образ без побочных действи
