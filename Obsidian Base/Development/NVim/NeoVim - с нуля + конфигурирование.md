...
Например:
- `:s/div/span` - заменит **в одной строке** тег `div` на `span`
- `:s/styles/aaa/g` - заменит значение 
...
2. **Глобальная замена в одной строке:**
    - Пример: `` — заменяет все вхождения "styles" в текущей строке на "aaa".
3. **Замена по всему буферу:**
    - Используйте `%s` для поиска и .
    - Пример:  — заменяет все вхождения "styles" на "aaa" во всех строках буфера.
