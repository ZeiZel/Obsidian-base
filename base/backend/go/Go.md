...
Строку с `export` следует поместить в `~/.zshrc`, `~/.bashrc` или другой файл конфигурации используемой оболочки. Перед заменой существующей установки лучше удалить именно каталог `/usr/local/go`, предварительно убедившись, что это нужная установка, а не произвольный пользовательский каталог.
