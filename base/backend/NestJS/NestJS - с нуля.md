...
И далее во вкладке _Memory_ можно сделать снепшот приложения на данный момент времени, который покажет все инстансы нашего приложения. На данный момент времени занято не так много ОЗУ, так как в приложение не поступало запросов
...
Чтобы исправить ситуацию, вместо обычного поиска по категории с параметрами, мы можем кастомно сагрегировать этот участок кода и воспользоваться `agregate`, который предоставляет методы для группировки.

- `match` соберёт нам все операции из первой группы
- `group` сгруппирует все найденные операции по полю `secondCategory`, что даст нам пустые группы
- собираем массив страниц `pages` через метод `$push`. `pages` будет содержать массив объектов с `title` группы и `alias`
...
async findByCategory(firstCategory: TopLevelCategory) {
	...
	return this.topPageModel
		.aggregate()
		// ищем сначала результаты по первой категории
		.match({
			firstCategory
		})
		// дальше производим группировку
		.group({
			// в качестве id группы будем использовать secondCategory
			_id: { secondCategory: '$secondCategory' },
			// собираем поле страниц
			pages: {
				// а уже тут в массив объекта страниц кладём нужные данные
				$push: {
					alias: '$alias',
					title: '$title'
				}
...
И теперь мы получаем массив со всеми сгруппированными категориями для отображения

![](../../_png/Pasted%20image%2020250801220834.png)

Так же есть другой способ написания агрегаций - через передачу массива параметров этих агрегаций

![](../../_png/Pasted%20image%2020250801221025.png)
...
Нам нужно будет подключить два внешних API для работы с данными, которые мы можем получить с _hh.ru_:
...
[Тут](https://github.com/hhru/api) находится документация по АПИ _hh.ru_
...
Тут мы описываем ответ по тем данным, которые приходят к нам с _hh.ru_
