...
`apps / account / src / app / user / user.m.ts`
```TS
import { Module } from '@nestjs/common';
import { MongooseModule } from '@nestjs/mongoose';
import { User, UserSchema } from './models/user.model';
...
Далее нужно интегрировать логику реализации под контроллер. Делаем это в сервисе. 
...
Далее подцепляем модуль JWT авторизации, контроллер и сервис.
