...
Опишем простой интерфейс курса без каких-либо дополнительных данных
...
Далее создаём контракт на получение курса. Здесь мы так же указываем топик для RMQ, запрос и ответ, который мы должны получить от сервиса.
...
Далее опишем контракт на генерацию ссылки на оплату с указанием топика, запроса и ответа, которые нам нужны. Генерировать оплату мы будем по кастомной сумме, так как у курса может быть любая цена из-за промо-акций и изменений цен, идентификатору пользователя, которму нужно будет отдать курс и, собственно, id курса, который нужно будет продать. 
...
Добавляем в `checkPayment` обязательное поле `status` 
...
export class BuyCourseSagaStateWaitingForPayment extends BuyCourseSagaState {
	...
	public async checkPayment(): Promise<ICheckPayment> {
		...
		// если просто неуспешно, то возвращаем пользователя
		if (status !== 'success') {
			return { user: this.saga.user };
		}
...
export class BuyCourseSagaStateCanceled extends BuyCourseSagaState {
	public pay(): Promise<{ paymentLink: string; user: UserEntity; }> {
		// возвращаем статус покупки в купленную
...
Опишем, инжектируемый в дерево зависимостей NestJS, эмиттер ивентов. Его основной задачей будет отправлять уведомления с данными по указанному топику

`apps / account / src / app / user / user.event-emmiter.ts`
```TS
...
В сущность пользователя теперь нужно будет добавить поле с ивентами и пушить события в очередь из метода установки нового статуса `setCourseStatus`
...
И отдельно реализуем сервис пользователя, в который перенесём бизнес-логику, которую недавно описали в контроллере.
...
Далее экспортируем в дерево зависимостей модули `UserEventEmiiter` и `UserService`, чтобы воспользоваться ими в другом модуле
