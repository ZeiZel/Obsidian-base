...
Описываем сагу начала покупки курса. 
...
Далее добавляем сагу `BuyCourseSagaStateStarted` в свитч основного состояния покупки курса клиентом в позицию старта
...
export class BuyCourseSaga {
	...
	setState(state: PurchaseState, courseId: string) {
		switch (state) {
			case PurchaseState.Started: // <-- 
				// устанавливаем стейт на данное условие свича в старт покупки
