...
- LWW - Last Write Win - если данные не важные, то можно сохранять в качестве "оригинала" последнюю внесённую запись
...
- Conflict-replicated data type (CRDT) - есть типы данных, которые не подвержены конфликтам
	- Дельта - если мы будем вносить в базу `+500` или `-500`, то мы будем получать дельту данных и при их сложении получать итоговый результат
...
У каждого шарда есть своя реплика, но шарды не реплицируется между друг другом, так как у них свои кусочки данных
