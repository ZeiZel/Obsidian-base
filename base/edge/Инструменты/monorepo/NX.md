...
NX - инструмент для управления monorepo. Он помогает держать несколько приложений и библиотек в одном репозитории, строить граф зависимостей, кэшировать сборки и запускать задачи только для затронутых проектов.

## Когда NX полезен

- несколько frontend/backend приложений в одном репозитории;
- общие TypeScript-библиотеки;
- единые правила lint/test/build;
- CI должен запускать только изменённые части;
- нужно видеть зависимости между модулями.
...
У нас в процессе создания монорепы будет возможность выбрать установку для одного проекта и для поддержки монорепозитория. Среда для одного проекта скорее нужна только для оптимизации загрузки пакетов, сборки билдов и CI/CD
...
# запуск цели у проекта
npx nx run admin:build

# запуск цели у всех проектов
npx nx run-many -t build

# запуск только затронутых проектов
...
В TypeScript-monorepo важно ограничивать импорты между слоями. Например, UI-библиотека не должна импортировать backend-код, а shared-утилиты не должны зависеть от конкретного приложения.

Обычно это контролируют через ESLint-правила NX и теги проектов.
...
Такие ограничения помогают не превратить monorepo в один большой неуправляемый проект.

## Плюсы

- граф проектов и зависимостей;
- affected-команды ускоряют CI;
- кэширование задач и генераторы для monorepo.

## Минусы

- добавляет платформенный слой и свои соглашения;
- для маленького проекта может быть избыточен;
- требует дисциплины в границах модулей.

## Когда использовать

- в репозитории несколько приложений и библиотек;
- нужно запускать lint/test/build только для изменённых проектов;
- команда готова поддерживать правила monorepo.
