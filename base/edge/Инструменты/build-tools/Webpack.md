...
Webpack - мощный сборщик модулей. Сейчас для новых frontend-приложений часто выбирают Vite или фреймворковый сборщик, но Webpack всё ещё очень важен в больших продуктах, legacy-проектах и сложных конфигурациях.

## Где Webpack полезен

- сложная настройка загрузчиков и плагинов;
- Module Federation;
- микрофронтенды;
- legacy-браузеры;
- проекты с большим количеством нестандартных asset pipeline;
- существующие React/Vue/Angular-приложения.
...
Для TypeScript обычно используют `ts-loader` или `babel-loader`/`swc-loader`.
...
Если хочется ускорить сборку, можно включить `transpileOnly` и вынести проверку типов в отдельный процесс через `fork-ts-checker-webpack-plugin` или отдельную CI-команду `tsc --noEmit`.
...
И теперь после подключения выходного файла к `index.html` webpack скомпилирует файл со всеми экспортами и импортами. Первыми в выходном файле всегда идут имитации экспортов и импортов и сами exports/imports, которые мы делали. Уже только потом идёт сам код.
...
- максимально гибкая система loaders/plugins;
- зрелая экосистема и много готовых решений;
- подходит для сложных legacy и enterprise-сборок.

## Минусы

- конфигурация быстро становится сложной;
- скорость ниже современных сборщиков в простых сценариях;
- для новых проектов часто избыточен.

## Когда использовать

- есть сложный asset pipeline или legacy-проект;
- нужны Module Federation или нестандартные плагины;
- команда уже поддерживает Webpack-конфигурацию.
