...
- **Команды** (отряд / squad). 
- **Чаптеры** (части). Группы по компетенциям в рамках команд. В каждой команде есть, например, QA, DevOps, Backend, Frontend. Для каждой части есть свой руководитель.
- **Гильдии**. Объединяют сотрудников по интересам на уровне всей компании. Этот срез нужен для коммуникации людей на уровне интересов, чтобы взаимно задавать вопросы и обучать друг друга
- Клан / Племя (департамент) (**tribe**). Содержит до 150 человек и объединяет команды с общей миссией.
- **Опыление**. Команды в Spotify достаточно автономны, но за счёт высокого уровня коммуникации, мы получаем повышение эффективности через обмен знаниями между командами.

![](_png/Pasted%20image%2020250103114621.png)

Но так же в Spotify есть:
- Релизные поезда - это процесс синхронизации релизов нескольких команд, которые выпускаются одновременно
