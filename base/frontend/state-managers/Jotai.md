...
[NPM: jotai](https://www.npmjs.com/package/jotai)
...
- атомарная модель снижает лишние re-render;
- минимальный API;
- хорошо компонуется через derived atoms.

## Минусы

- сложно увидеть общую картину состояния при большом числе атомов;
- архитектура атомов требует соглашений;
- для event-driven бизнес-логики может быть менее явным.

## Когда использовать

- состояние естественно раскладывается на независимые атомы;
- нужно заменить множество `useState`/`useContext`;
- важна точечная подписка компонентов на данные.
