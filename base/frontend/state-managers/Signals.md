...
[NPM: @preact/signals-react](https://www.npmjs.com/package/@preact/signals-react)
...
- тонкогранулярные обновления;
- простая модель signal/computed/effect;
- хорошо подходит для частых мелких изменений.

## Минусы

- в React это внешний реактивный слой;
- нужно следить за интеграцией с React-паттернами;
- экосистема меньше, чем у классических state managers.

## Когда использовать

- нужны очень частые точечные обновления UI;
- проект использует Preact/Solid/Angular signals или экспериментирует с реактивностью;
- состояние должно обновлять минимальную часть интерфейса.
