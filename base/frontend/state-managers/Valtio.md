...
[NPM: valtio](https://www.npmjs.com/package/valtio)
...
- мутабельный API поверх Proxy;
- удобен для вложенных объектов;
- компоненты подписываются на используемые поля.

## Минусы

- Proxy-магия может быть непривычной;
- нужна аккуратность со снапшотами;
- не лучший выбор для строгого event-driven flow.

## Когда использовать

- состояние похоже на дерево объектов;
- команда хочет писать мутации напрямую;
- нужно быстро управлять вложенными структурами без reducers.
