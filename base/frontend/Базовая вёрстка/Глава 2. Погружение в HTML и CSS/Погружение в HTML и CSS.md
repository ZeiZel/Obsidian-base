...
При использовании данной инструкции, у нас все `padding` и `margin` будут учитываться в размере элемента.
...
По макету мы можем выставить отталкивание в пикселях, но так же нам никто не мешает посчитать %-ное соотношение между шириной макета и положением блока (положение*блока / ширина*макета)
...
Меняем шрифт через `font-family`. Если название шрифта состоит из одного слова, то можно написать без кавычек. Если из двух, то с кавычками. Посмотреть шрифт можно прямо в инспекторе (там же можно найти в `font-weight` во вкладке _computed_). Шрифты берутся из базы шрифтов на нашем компьютере (по-другому мы их не увидим)
