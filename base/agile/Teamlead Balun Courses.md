...
>[!info] Книга
> Дэвид Ален - Как привести дела в порядок. Искусство продуктивности без стресса. 
> Она описывает техники пустого инбокса и списка задач. 
> Краткое описание: 
...
![](../_png/Pasted%20image%2020260809103157.png)
...
- Что я делал? Output - это результат, который можно пощупать 
- Что я сделал? Outcome - это ценность, которую мы принесли своим результатом 
- Что было не так?
- Что стоит улучшить?
...
>[!question] А как же освободить ещё больше времени? 
...
>[!info] Японская техника "5 почему" позволяет дойти до реального ответа. 
...
- Руководитель не занимается своими прямыми обязанностями, которые от него ожидает его начальство 
...
> Мы делегируем принятие решения, но мы остаёмся ответственными, как руководитель. 

Как делегировать: 

1. Берём инструмент планирования. 
2. Выбираем те задачи, которые объективно можем выполнить только мы. На нас всегда только 3 приоритета и всё остальное делегируем. 
3. Для остальных задач выбираем сотрудников, которые могут решить задачу (например, у них больше экспертизы или у них есть запрос на рост). 
4. Решаем, какие вводные нужны, чтобы выполнить задачу. 
5. Отправляемся к выбранному сотруднику, объясняем все, что необходимо, и ставим срок, к которому должен быть получен результат. Сотрудник должен задавать вопросы, если что-то не понял. 
...
- Контроль
	- Нужно контролировать выполнение периодически и не забывать про задачу, потому что её выполнение отложат, если про неё забыть 

Ошибки делегирования:

1. Бояться, что ничего не сделают. 
2. Не возвращаться в срок. 
   Нужно каждый отрезок времени выполнения задачи уточнять процесс её работы. 
3. Не давать вводные. 
4. Задавать сроки директивно. 
   Нужно обсудить сроки, когда получится выполнить и какие оцениваются сложности. 
5. Не отвечать на вопрос "Зачем?". 
   Нужно, чтобы команда понимала, для чего она это делает. 

### Тактические и стратегические задачи

>[!book] Глеб Архангельский - Тайм Драйв 
...
В них, например, можно формировать задачи на квартал, которые мы будем разрабатывать. Есть большие связанные задачи, которые мы можем делить по тегам и определять свой цветовой код. 

![](../_png/Pasted%20image%2020260607213731.png)

MindMap. 

Глобальные задачи, которые нужно выполнять и держать на виду, чтобы не терять контекст и мозгофлоу. 

![](../_png/Pasted%20image%2020260607213747.png)

Построение простой диаграммы с нашим шаблоном для удобного начала работы. 

Тут выглядит задачник примерно так же, как и диаграмма по столбцам с задчами, где у нас задачи распределяются по треугольнику. Чем дальше мы прогнозируем конкретные задачи, тем менее детализированными они должны быть. 

![](../_png/Pasted%20image%2020260607213952.png)

>[!book] Так же есть [Roadmap продуктов по O'Relly](https://www.oreilly.com/videos/product-roadmaps-101/9781806389773/), где описаны дополнительные подходы к построению инструментария. 
...
![](../_png/Pasted%20image%2020260830141633.png)

В итоге у нас должен сложиться пайплайн: 

- Общение
	- Постановка задачи
- Выявление сути - никогда не берём задачу до тех пор, пока не поймём, чего конкретно от нас хотят
	- Корректировка задачи
	- Декомпозиция
- Обозначение сроков
	- Сроки - если мы взялись покрасить стену и сразу выдали срок, а по факту оказалось, что стены в доме заказчика идут волной
	- Сжатие задачи
...
3. Есть чёткий результат, которого нужно достичь 
4. Помогает команде достичь целей 

Как спланировать эффективную встречу (любой sync): 

1. До встречи
	1. Обозначить цели встречи (с аджендой и повесткой в описании)
	2. Приготовить повестку 
	3. Разослать повестку всем участникам 
2. Во время встречи
	1. Распределить роли: фасилитатор (поддерживает повестку встречи и держит всех в рамках неё), Time Keeper (мы должны уложиться в час), Note Keeper (нужна сборка всех фактов)
	2. Придерживаться повестки
	3. Записывать (нужно сохранять повестку встречи, так как сразу после неё мы запоминаем 80%, а к концу дня только 20%)  
3. После встречи
...
>[!example] Ревью операционных метрик 
> - Зачем: 
> 	- Позволяют оценивать качество нашей работы 
> 	- Если метрики просядут, то команда получит по шапке 
> - Есть регламент, по каким необходимым метрикам проходимся и что нужно подтянуть 
> - Фасилитатор в лице тимлида 

>[!info] Как проводить ретро
> - Выделять не менее 1,5 часов на него 
> - Применять фреймворк BSG 
> 	- Bad 
> 		- бесит и прямо печёт
> 		- 10 минут заполнения
> 		- 20 минут объяснение 
> 		- формирование action items и ответственных с конкретными сроками 
> 	- Sad 
> 		- грустно, непонятно и стоит улучшить
> 	- Glad 
> 		- отлично, что получается в команде 
> - Тут очень важен подход "бутерброда с говном" - на входе позитив, в процессе грустную суть и на выходе позитив 

>[!info] Как проводить демо
> - строго 15 минут на команду 
> - что делал, что сделал и обозначает проблему 
> - описывать проблемы не нужно - это отдельная встреча 

>[!book] Джим Кэмп - Сначала говорите нет 
> - Отказ от нужды: Нельзя показывать оппоненту свою зависимость от заключения сделки. 
> - Фокус на процессе: Важно управлять своими действиями в моменте, а не загадывать результат. 
> - Мир партнера: Нужно вести диалог, опираясь на интересы и видение второй стороны. 
...
Слева у нас находится **команда** - **Team**|**Lead** - **руководство** справа. Teamlead выступает зачастую мостом между командой и руководством. 

Мы должны продавать идеи и подходы сразу команде и руководству: 
...
> Вопросы и там и там абсолютно одинаковые, но меняется контекст их выяснения. 

И с той, и с другой стороны будут недопонимания: 

- Одних постоянно подгоняют сроками 
- А вторые говорят, что айтишники сидят на зарплате и ничего не делают 

И по сути: 

- Команда хочет брать конечное ТЗ, разобраться в нём и просто выполнить задачу в срок 
- И бизнес хочет дать задачу, чтобы команда взяла её и выполнила предсказуемый результат в срок 

> И с той, и с другой стороны разные потребности, но контекст их меняет в корне. 
> Позиция одинаковая, но контекст меняет её координально. 

![](../_png/Pasted%20image%2020260608223116.png)

>[!example] У нас есть задача: сделать зелёную кнопку
> Мы её сделали и раскатили на всех окружениях. 
> Потом к нам приходит бизнес и говорит, что кнопка должна быть зелёной и со скруглениями. 
...
>[!cite] Мне неважно, почему "нет". Я хочу знать, что сделано, чтобы было "да"!

В этой ситуации важно ли, кто виноват конкретно, что задача была сделана не до конца некорректно? Это не важно. Нам важно в моменте прийти к бизнесу и уточнить, что нам нужно сделать, чтобы эту задачу сделать корректно. То есть нам нужно сформировать более чёткие требования. 

Тимлид в первую очередь не ищет виноватых, а ищет проблему. 

Задача найти, почему у нас не получилось - это ретроспектива. В моменте нам самое главное доставить ценность. 

>[!warning] Огромная доля ошибок вызвана недопониманием ожиданий

В компаниях часто менеджмент торопит разработчиков и не даёт писать RFC/ADR по задчам. То есть все доработки должны нести конкретный характер, когда мы погрузились в задачу и проработали с бизнесом все варианты работы фичи. 

RFC/ADR пишется разработчиком и основывается на техническом решении со всеми ограничениями НА базе ТЗ, которое составил бизнес и аналитик. 

Если этого НЕ делать, то мы каждую итерацию будем приносить не то и однозначно выгорим. 

>[!important] Чем точнее мы понимаем ожидания, тем меньше нужно будет дорабатывать потом и, соответственно, меньше делать лишней работы.  

![](../_png/Pasted%20image%2020260608223211.png)

>[!info] Сколько раз в неделю приходится переделывать результат из-за того, что что-то недопоняли? 
...
>[!warning] Глупых вопросов не бывает - лучше уточнить, чтобы не сделать фигню. 
...
>[!example] Если мы бегаем по утрам, а остальные нет, то это не потому что все лентяи, а я хороший. Причин разных может быть много: кто-то ходит вечером в зал, много гуляет в целом или, да, просто ленится. 

#### Позиционное мышление

Для того, чтобы договориться, надо понять, чего хочет твой оппонент

>[!info] Чтобы быть понятым и доносить свои мысли, нужно выделить в себе силы взглянуть на ситуацию со стороны нашего оппонента 
> Нам нужно продавать ручку не от того, почему она такая классная, а отталкиваясь от того, зачем она ему. 

![](../_png/Pasted%20image%2020260608223610.png)

##### Сдвиг парадигмы 

Когда мы включаем позционное мышление, мы всегда должны сдвигать наши устои в голове. 

Мы видим ситуацию со своей стороны -> мы считаем, что мы правы -> получаем важный контекст -> включается позиционное мышление. 
...
> Нельзя подходить к вопросу так, что нам нужно просто выделить 20% времени на рефакторинг. 

> Нужно подойти и обосновать: 
> - повысится отказоустойчивость 
> - ускорится поставка продукта на стенды 
...
- Ошибка: начинаешь с вершины, а не с основания - нужно объяснить, зачем команде вообще нужно делать эту задачу 
...
1. Situation - 1 предложение - что есть сейчас и в чём проблема 
2. Complication - почему это проблема? - объяснить, что делает ситуацию проблемой 
3. Question - что решаем?
4. Answer - решение - нужно предложить решение 
...
>[!warning] Но этот подход не покрывает полностью все кейсы. Он не отвечает на глубокие вопросы "почему"? Поэтому нужно использовать MECE. 
...
> То есть каждый пункт проблемы/цели по MECE должен быть атомарным и изолирован в своём домене 
...
Далее сформируем заново наше предложение: 
...
Complication: 
Сейчас деплойим раз в 2 недели, покрытие тестов 65%. Вчера упали платежи на 2 часа, потеряли 1.2млн руб. 
Question: 
Как я хочу сократить цикл деплоя до 1 раза в неделю?
...
1. Такие решения будут челленджить - они не конечны. Команда придёт на встречу и у неё уже будут мысли в голове насчёт решений. 
2. Но теперь разговор строится от конкретных шагов, а не от абстрактного понимания с точки зрения каждого. То есть мы берём не потому что "стильно, модно, молодёжно", а опираясь на решение команды со всеми плюсами и минусами выбранного подхода. 
...
- Решение - простыми словами объясняем решение 
- Результат - цифры через месяц 
- Риски - что может пойти не так 

> Если сделаем ЭТО, то получим вот ЭТО. Если не сделаем, то потеряем вот ЭТО. 
...
- Результат: Через месяц стабильность вырастет, снизится нагрузка на саппорт 
...
> Не обязательно даже разъяснять, что конкретно будет исправляться. Владельцу ресурса важно объяснить понятный результат, к которому мы должны прийти, если примем это решение. 
...
- Хорошо: "Нужны метрики загрузки сервера к 15:00. Формат CSV, топ-5 пиков. Успеешь?"
	- Есть ситуация 
	- Конкретизируем срок 
	- Уточняем реальность получения данных 
...
4. Если началась полемика, то перечитайте сообщение ещё раз. Выдохните. И ещё раз перечитайте, прежде чем отправить
...
5. Избегайте двусмысленностей. Выражайте мысли так, чтобы они были однозначно понятны 
...
> Проблема: люди есть, а команды нет. Не смотря на то, что цель может быть одна и та же у всех. 
...
| Критерий                                   | DoR                                                                                         |                                                                                            |
| ------------------------------------------ | ------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------ |
| Группа людей                               | Вначале всегда появляется некая группа людей, работающая над чем-то в схожей области        | Есть всегда                                                                                |
| Есть правила работы                        | Люди стремятся к правилам, чтобы не сталкиваться лбами                                      | определение ответственности за каждый кусочек работы                                       |
| Есть общая цель                            | Люди начинают понимать, что делают вместе что-то одно                                       | Правила работать без этого не будут                                                        |
| Есть зоны ответственности                  | Люди понимают, что толкаться локтями и делать одно и то же - неприкольно                    | если тимлид справляется с оркестрированием и фасилитацией, то отбирать у него это не нужно |
| Есть роли                                  | Помимо hands-on работы люди принимают на себя поведенческие и профессиональные роли         | Люди следуют ей и выполняют задачи исходя из мотивации этих ролей                          |
| Есть личная и коллективная ответственность | Люди понимают, что несделанная ими работа - это провал                                      | Общая ответственность                                                                      |
| Есть самостоятельность в работе            | Работают взрослые люди, которым не надо напоминать, что работа должна быть сделана [хорошо] | Самостоятельность                                                                          |
...
| Размер | Микрокоманда - 2-4 человека                                    | Оптимальный размер - 5-9 человек                                       | Большая команда - 10+ человек                                   |
| ------ | -------------------------------------------------------------- | ---------------------------------------------------------------------- | --------------------------------------------------------------- |
| Плюсы  | - Быстрые решения<br>- Высокое доверие<br>- Легкая координация | - Оптимальный баланс<br>- Разнообразие ролей<br>- Управляемая динамика | - Широкие компетенции<br>- Высокий ресурс                       |
| Минусы | - Узкий набор навыков<br>- Перегруз ключевых людей             | - Нужны явные нормы<br>- Важна роль лидера                             | - Много коммуникаций<br>- Риск подгрупп<br>- Замедление решений |
...
1. ==Улучшенная коммуникация==. В небольших командах меньше каналов связи, что позволяет быстрее принимать решения. Меньше споров + быстрее координация. 
2. ==Повышение производительности==. Это смягчает эффект Рингельмана, при котором индивидуальная производительность снижается в больших группах. Если у нас в команде 15 и мы добавляем ещё 15, то производительность увеличится только в 1.3-1.5 раза. 
...
- Важно проработать базовые правила и процессы, чтобы команда работала активно 
...
- Поддержать нормы, поощрять открытый диалог, хвалить людей (проводить тимбилдинги) 
...
1. Ориентация. Что я здесь делаю? Формиируем совместную работу. 
2. Одобрение доверия. Кто ты? 
3. Уточнение цели. Что мы делаем? 
4. Обязательность. Как мы это сделаем? 
5. Распределение ролей. Кто, как, что, когда? 
6. Высокая производительность. Ура! 
7. Обновление. Зачем продолжать? В какой-то момент у команды наступает рутина. Этот этап нужен, чтобы команда не закислялась. 
...
- Резкое изменение задач / приоритетов
	- Неопределённость и постоянное изменение приоритетов сбивает столку команду и перф падает 
...
- девопс 
- спецлиалист по технической поддержке 
...
- `- shaped` - специалист широкого профиля - поверхностно разбирается во многих областях, но ни в одной не является экспертом
...
> Поэтому не очень круто, когда: 
> - продутк овнеру подчиняется техническая команда 
> - техдир подчиняется продуктдиру 
> Потому что у них изначально заложен конфлит интересов: продукт всегда хочет получить быстрый продукт на рынке, а тех хочет, чтобы система не развалилась. 
...
| Функция                                           | Роль                                       |
| ------------------------------------------------- | ------------------------------------------ |
| Руководство командой, организация работы - тимлид | Мотиватор,<br>Координатор,<br>Душа команды |
...
> Если какой-то роли нет для определённой позиции, то её нужно будет делегировать 
...
> Нужно стараться в команде все 9 ролей имплементировать 

Результаты: 

1. Наилучшие результаты - у хорошо сбалансированных по ролям команд
2. Однородные команды малоэффективны (если все в команде педанты, то разработка будет сильно замедляться) 
3. Отсутствие какой-либо роли - слабость команды 
4. Недостатки несбалансированных команд могут быть скомпенсированы за счет самопознания (можно вырастить человека и наделить его ролью) 
5. Важно учитывать взаимодействие ролей и связи с позициями в структуре организации (делать из джуна фасилитатора - это плохая идея) 
...
- Бежевый - старт команды, стартап, мало денег 
- Фиолетовый - появляются клиенты, налаживается первый поток денег, "мы семья" 
- Красный - выделяется властный лидер, который берёт в свои руки развитие 
- Синий - переход от лидера в закон и порядок системы (армия) 
- Оранжевый - нарушение правил и переход к результатам, (либо огребаем, либо получаем "спасибо"), появление лидеров 
- Зелёный - agile, командое обучение, нормальные HR 
- Жёлтый - переход к развитию каждого человека и системе 
- Бирюзовый - стремление на глобальный вклад в индустрию и поколения (valve) 
...
> Цвет может опускаться в рамках одной компании с другим цветом 
...
- Сеньоры и руководители - меньше текучки, чем у мидлов, так как их предложения, обычно, сильно лучше 
...
Да, все горит. Но у вас есть кард-бланш и можно внедрять ровно то, чего команде не хватает. 

> Если таких полномочий не дают, срочно пишите "по собственному"
...
В такой команде можно сгореть по нескольким причинам: 

- Нам не дают ничего делать 
- Мы хватаемся за всё 
...
- Оцените тип своей команды по [cynefin](agile.md#Cynefin)
...
> Люди не доверяют друг другу и системе достаточно, чтобы эти процессы работали. Это формирует культуру невосприимчивости. 
...
- Плохие процессы в хорошей среде - исправляются или уходят, потому что люди не будут бояться говорить о проблемах и вместе их решать 

##### Культура складывается каждый день

- Нельзя провести тимбилдинг и ожидать, что культура будет огонь
- Культура складывается из сотен маленьких реакций тимлида и команды каждый день, поэтому важно следить за своими реакциями 
...
>[!success] Доброе слово и кошке приятно 
> Мы часто забываем говорить людям, что они поработали хорошо. 
> Это обязательная часть работы, чтобы ему было приятно и он чувствовал себя компетентным. 
...
>[!info] "Молодец, задача сдана" vs "Молодец, что предупредил заранее о риске"
> Нужно хвалить не только за достижения, но и за его правильное поведение в определённой ситуации. 

Поведение в стрессе: 

- Когда горит дедлайн, вы становитесь резким и начинаете микроменеджить?

>[!warning] Команда лучше запоминает стрессовые моменты, чем спокойные 
> Если мы в стрессе сядем вместе с командой и определим, как нужно чинить ситуацию, но без коучинга и директивно, сказать всем, что делать и в какие сроки, то это будет лучше, чем всех подпинывать и нервничать. 
> Команда поймёт, что у неё есть лидер, которого не пугают проблемы. 
...
1. Подготовка к коммуникации
	1. Структурированность информации (MECE и подобные техники)
	2. Единое понятийное пространство (разные термины мы можем интерпретировать по-своему) 
	3. Понимание уровня взаимоотношений (без фамильярного отношения на работе) 
2. Ключевое условие
...
3. Конструктивная подача
	...
	2. Адресность
	3. Факты и данные
	4. Решение проблем, а не человека (нужно говорить явно о проблеме) 
...
> И это меняется гораздо чаще, чем раз в квартал, так как у людей постоянно в жизни происходят разные вещи. 

##### Как узнать? 

- 30 минут раз в неделю лучше, чем час раз в месяц
...
1. Встреча превратилась в статус по задачам
	...
	2. Если мы не будем переходить на человека, то каждая встреча будет описанием выполненных рабочих задач на этой неделе. Нам же нужно строить доверительное отношение и следить за состоянием члена команды. 
2. Человек не доверяет достаточно, чтобы говорить честно
	1. Молчание на 1-1 часто значит "я не уверен, что это безопасно сейчас сказать". Особенно в начале
...
3. Встречи слишком редкие
	...
	2. При еженедельных встречах темы появляются естественно и сама встреча превращается в доверительную беседу, где не страшно говорить о проблемах 
...
- Но сначала дайте ему пространство, ему нужно дать выговориться 
...
- Принудительный боулинг. 
	- Если половина команды интроверты - они страдают, но улыбаются. Эффект обратный. 
- Корпоратив раз в год. 
	- Люди напиваются, танцуют, а в понедельник всё как было. Культура не живёт в одном событии. 
- Мероприятия без учёта людей. 
	- Спросите команду: что вы хотите делать вместе? Может, они хотят просто поиграть в настолки или сходить на квиз, а не в верёвочный парк. 
...
- Обоснование - нужно явно обосновать, где не хватает ресурса и зачем нужен новый человек 
- Портрет - что это за человек 
- Описание вакансии 
- Этапы собеседований 
- Оффер 
- Испытательный срок 
...
> Поэтому такой аргумент будет несостоятельным, так как если задач будет меньше объёма команды, то большой размер команды будет не нужен 

#### Моделирование команды

- Список CJM - список действий пользователя на площадке 
- Бэклог - из CJM формируется бэклог 
- Экспертная оценка - потом производится оценка выполнения задачи определённым человеком 
- Проектный план - потом по оценке формируется обший план и на квартал в ближайшем приближении 
- Оптимальный план - немного растянутый предыдущий план с включённым гэпом под риски и переформированный под сроки от бизнеса. Если нужно, чтобы команда успела быстрее, то нужно больше человек, которым можно будет раздать больше задач. 
...
- ЗП net - 300 000 рублей на руки в месяц
- Gross – 344 827 рублей
- Отчисления – 452 724 рубля в месяц
- Стоимость найма – от 3-х ЗП
	- нужно вакансию разместить 
	- нужно кандидатов отсобеседовать 
	- нужно оборудование 
	- нужен онбординг 

>[!warning] 6 338 136 рублей в год 
> И это только один человек! 
...
Вакансия сразу про агрессию и сравнивает себя с кем-то + ставит условия по зарплате - плавает 

![](../_png/Pasted%20image%2020260623151657.png)

В вакансии нет описания цели, что нужно делать. Просто постоянная работа, работа и работа. 
...
> Но большинство людей идут за взаимодействием и опытом: с кем, над каким проектом, в какой среде будет идти работа 
...
4. Следите за рынком и ценами (взять сеньёра за $1500 сейчас невозможно) 
5. Не лгите! 
6. Ваша компания не Google* 
7. Не поленитесь нажать F7 (проверить орфографию) 
...
- Ищите везде 
- Поиск продолжает и дополняет описание (если выкладываемся в тг-каналах и других источниках, то нужно следить за тем, что там пишут) 
- Забудьте про большой HR-аутсорс 
- Никаких Tinder! 
- Только честная информация (работает в обе стороны) 
- Пользуйтесь внешним опытом (если нет опыта работы с определённым стеком, то стоит заплатить за знающий внешний опыт) 
...
Воронка найма: из 1000 просмотров и 300 откликов -> 2 оффера 
...
- Чем Вы гордитесь? <- хороший вопрос 
...
Нужно иметь список схожих вопросов. У нас должен быть чеклист вопросов кандидату, по которым мы явно будем проходиться. Только так можно сравнить двух кандидатов и проверить, подходят ли они нам или нет. 

![](../_png/Pasted%20image%2020260915192640.png)
...
>[!example] Например, можно дать плохой код и попросить объяснить, в чём заключается нарушение LSP по SOLID 
...
1. Сначала мы готовимся к найму. Все согласования, вакансия, требования, место. 
2. Если мы ищем хотя бы миддла, то дальше идёт 2-3 месяца поиска сотрудника и собеседования. 
3. Далее кандидату нужно принять оффер и отработать 2 недели на прошлом месте работы. 
4. Потом начинается старт работа и испытательный срок. 
...
>[!example] На собеседовании мы обещали Highload и 500k запросов в секунду 
> - Вот тут в прод базу нужно сходить руками 
> - Тут логов и трейсов и не планировалось 
> - Вот тут через excel приходят нам данные, которые нужно загружать 
...
- Сотрудник должен понимать критерии успеха с первого дня 
...
- Если есть проблема - говорим сразу и без экивоков (основная цель - совместно найти решение проблемы) 
...
Если у разработчика это происходит на 2-3 месяц, то в среднем для руководителей это происходит на 6-й месяц, по мнению М. Уоткинса. 

>[!important] Задача: сократить этот срок
...
> Нужно дать человеку пространство за испытательный скрок сделать понятный инкремент 
...
##### `[Тк РФ, ст. 71]` Результат испытания при приёме на работу: 
...
Очень сложно формализовать полномочия, обязанности и требования в IT: 
...
> Этот набор нужно подготовить заранее ещё до того, как он выйдет на работу 
...
- Демо продукта с командой (не в записи, а живое) 
	- Команда показывает основные CJM в реальном времени - не на видеозаписи, а живым прогоном по фичам прямо на демо 
	- Заодно проверят стабильность 
- Eat your dog’s food 
...
| Ментор   | Опытный коллега    | Развивает профессионально, долгосрочно, выбирается после ИС    |
| Buddy    | Коллега из команды | Отвечает на рутинные вопросы, помогает освоиться, на время ИС  |
...
- Coffee chat - 15 мин с каждым членом команды в первые 2 недели познакомиться (Привет, я _Олег_ и делаю _сервис оплаты_) 
...
>[!book] Will Larson - An elegant puzzle 
...
- Задаёт уточняющие вопросы, а не базовые (например, подсвечивает corner cases) 
...
| ------- | ------------------------ | --------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| R1      | Новичок-энтузиаст        | S1        | Директивный: <br>- Много указаний, мало поддержки<br>- Менеджер говорит что, как и когда делать<br>- Пример: "Сделай вот это, вот так, к пятнице"<br>- Новичок хочет работать, но не знает как - ему нужна структура, а не автономия                                               |
| R2      | Разочаровавшийся новичок | S2        | Коучинговый: <br>- Много указаний + много поддержки<br>- Менеджер объясняет решения, вовлекает в диалог<br>- Пример: "Давай разберём вместе, почему я предлагаю именно этот подход"<br>- Человек уже столкнулся с трудностями, энтузиазм упал - нужно и направлять, и поддерживать |
| R3      | Опытный, но неуверенный  | S3        | Поддерживающий: <br>- Мало указаний + много поддержки<br>- Менеджер помогает, но не диктует<br>- Пример: "Ты знаешь как - я рядом, если нужно"<br>- Человек умеет, но сомневается в себе нужна уверенность, а не инструкции                                                        |
| R4      | Мастер с мотивацией      | S4        | Делегирующий: <br>- Мало указаний + мало поддержки<br>- Менеджер ставит цель и уходит<br>- Пример: "Вот результат, который нам нужен - ты знаешь что делать"<br>- Человек и умеет, и хочет — избыточный контроль его демотивирует                                                  |
...
- Цели на следующий квартал (если есть минусы на ИС, то определяемся, как мы можем их исправить и нарастить опыта) 
...
>[!note] Radical Candor требует говорить сразу и по-маленьку - не копить. 
> Тогда на человека не свалится снежный ком из проблем. Если работа сделана плохо, то об этом нужно сказать, чтобы её переделали. Не нужно этого бояться. 
...
>[!warning] Если не будем выстраивать доверительные отношения, то мы будем терять уйму полезной информации 
...
> Если нам нужна экспертиза человека, то можно попробовать договориться, чтобы по ГПХ передавать знания по проекту 
...
> Если уходит тимлид из команды, то с ней должен поговорить менеджер тимлида, чтобы объяснить ситуацию 
...
>[!book] "Радикальная прямота" - Ким Скотт
...
>[!example] Несоответствие работы кодстайлу 
...
>[!book] 45 татуировок менеджера - Максим Батырёв
...
> Пришёл ли я к человеку с персональным планом, сроками и ясными требованиями? 
...
>[!warning] Но если мы сокращаем человека, то на ту же самую должность нельзя будет нанимать человека в течение года и нужно будет придумывать новую позицию в штате.  
...
>[!example] Когда здесь будет конфликт, а когда — девиация?
...
> Конфликт - это не ссора. Это столкновение интересов. 
> Ссора - это одна из форм проявления конфликта. 
...
>[!example] Нам вредны деструктивные и беспредметные конфликты: 
> - Мне не нравится этот парень, он меня бесит. 

>[!example] Зал или кино - что выбрать? 
...
>[!example] На кухне лежит пицца, но мы только что пообедали. В целом, нам без разницы, съест ли человек последний кусок или нет. 
...
>[!example] Мы запланировали релиз. К нам приходит разработчик и говорит, что у нас в определённом месте может вылезти ошибка на проде. Изначально в нашей голове всё нормально, но мы ловим себя на мысли, что да, нужно повременить с релизом и подождать решения проблемы. 
...
>[!success] "Я чувствую давление, когда так говорят. Давай попробуем разобраться в причинах, почему так случилось."
...
>[!example] На ретро участник говорит: "Да всё это бесполезно. Мы каждый раз обсуждаем одно и то же, ничего не меняется"
...
> 3. Хорошо, давай я до следующего ретро схожу в ... и узнаю, что нам мешает сделать так, как мы хотим. Я попробую доработать наш процесс таким образом, что бы он встроился к нам, либо попробую выбить бюджет на доработки. 
> 4. Главное после этого не забить - иначе девиацию не вылечим. 
...
> Зачем мы это делаем? Это же полная фигня? 

Манипулятор всегда будет выводить на эмоцию. 

![](../_png/Pasted%20image%2020260620171914.png)

Вывод на лёгкую эмоцию

>[!example] Вот там пришёл злой менеджер, но ты же жертва, давай я тебе помогу.
> Спасатель всегда манипулирует жертвой.   

>[!example] Самое бунтарское, что ты можешь сделать - это... Меня зажали в рамки! На самом деле я крутой чувак! 
> Тут нужно остановиться и понять, зачем нам бунтовать вообще?  
...
Чтобы не подвергаться манипуляциям, когда нам их накинули, нужно: 

1. Выдержать небольшую паузу 10-12 минут и выпить чая 
2. Далее нужно вернуться к фактам 

Как выглядит Конструктивная конфронтация 

>[!example] У нас ничего опять не получится все козлы
> - Нас пытаются вывести на эмоцию 
> - Но нам нужно вернуться к данным и фактам, почему так происходило 

![](../_png/Pasted%20image%2020260620172554.png)

Принципы конструктивизма: 

1. Своевременность - проблема не должна быть в прошлом - она должна быть насущной 
2. Адресность - мы говорим о конкретной проблеме  
3. Данные и факты - всегда отсылаемся к определённым данным и фактам 
4. Фокус на проблеме

### Конфликт как ресурс развития

>[!example] Вспомните конфликт, который в итоге дал что-то хорошее команде 
...
>[!success] Но есть и другой угол зрения. 
...
> - Нужно выработать регламент написания тестов. 

>[!success] Конфликт сделал проблему видимой. Без него она бы продолжала тлеть. 
> То есть нам нужно тушить не сам конфликт, а копнуть вглубь и понять, почему конфликт произошёл. 
...
>[!example] Если человек не меняется при девиантном поведении и продолжает идти на кофликт. 
> Я попытался тебе объяснить, но ты продолжаешь делать так же. В нашем коллективе это неприемлемо. Поэтому давай договоримся: либо ты попытаешься выйти со мной на конструктив, либо нам придётся расстаться. Потому что я не могу позволить таким эмоциям разрушать команду. Мне команда априори дороже одного человека. 
...
В жизни нам постоянно приходится принимать различные решения. Мы, как тимлиды, должны их принимать каждый день. У каждого решения есть своя цена и вес. 

>[!note] Торговец бороздит океан 
> Перед ним встал выбор: отправить свой корабль через море и получить прибыль, НО с вероятностью 50/50 корабль разобьётся, команда погибнет и всё добро утонет. 
> Проблема ещё заключается в том, что кораблю нужно будет вернуться обратно и шанс так же 50/50, что в итоге будет равняться только 25% шанса на успешный исход всего действия.  
> Тут мы сталкиваемся с тем, что любое решение торговца будет неэффективным с точки зрения менеджмента. 

> 50/50 для менеджера - это везение, а не гарантированный результат 

#### То, что очевидно тебе - неочевидно другим

То, что понимаем мы и наш собеседник проходит через наши собственные опыт, факты и точку зрения. 
...
- Ты - погружен 
- Остальные этим вопросом не занимались 
...
Когда ты что-то знаешь -> ты не можешь представить, каково это - не знать. У нас уже сформировались нейронные связи, которые приводят к тому, что оно кажется само собой разумеющееся. 
...
Контекст наш и стейкхолдеров сильно отличается. Если мы придём с потребностью отрефакторить сервис, зная все его проблемы, у второй стороны будет вопрос: зачем в это вкладывать ресурсы, время и деньги. Нужно презентовать проблемы. 
...
- Понимает, что можно сделать лучше с позиции инженера, поэтому мы принимаем либо отклоняем, либо критикуем решение 

Даже решение команды является нашим решением, которые мы должны уметь валидировать и презентовать стейкхолдерам. 

Обычно путь от выполнения задачи исполнителем до конечной цели крайне тернист и угловат. 

![](../_png/Pasted%20image%2020260623194649.png)

Наша цель, как тимлида, сделать путь максимально прямым. 

Решение призвано выбрать оптимальную опцию для достижения цели минимальными затратами, но с максимальным качеством. 

![](../_png/Pasted%20image%2020260623194709.png)

>[!warning] Мы приходим к результату по-разному: 
...
>[!warning] Риски - потенциально существующая возможность потери ресурсов в результате реализации решения aka чёрный лебедь 

>[!important] Неопределённость
...
> Проработка решений несёт собой не только новые риски, но и определённое количество неопределённостей, которые нужно разрешать 
...
Наша основная цель в этой задаче - принять решение с минимальной неопределённостью и минимальным временем. Пока мы принимаем решение о том, как внедрять ERP, компания будет терять время и деньги. Если старые процессы не перестроить на единую систему ERP, то долг миграции будет продолжать расти. 

В точке пересечения линий графика мы уже дошли до минимального уровня выработки решения и понимаем, с чем можем стартануть. Тут рождается состояние MVP. 

![](../_png/Pasted%20image%2020260623232551.png)

>[!example] Вместо того, чтобы сразу внедрять в нашу систему автоматизации сложную схему подписания документов, мы можем внедрить простой метод неквалифицированной подписи. Она не закроет всех проблем, но уже начнёт приносить ценность. 

#### Процесс достижения результата

> Проблема -> Требования -> Цель -> Решение -> Исполнение -> Результат

![](../_png/Pasted%20image%2020260916194631.png)

Способы собрать требования: 

##### ITIL 4 Continual Improvement Model

ITIL 4 CIM — это пошаговая схема непрерывного улучшения процессов, сервисов и способов работы: от понимания текущего состояния до проверки результата. 

Сначала определяют цель, оценивают, где находятся сейчас, задают измеримые показатели, составляют план и выполняют изменения, после чего проверяют, достигнут ли нужный результат. Затем цикл повторяется: успешные изменения закрепляют, а следующие улучшения планируют на основе новых данных.

- What is the vision 
- Where are we now - сбор требований и формирование качественных показателей 

![](../_png/Pasted%20image%2020260623232805.png)

На этапе What is the vision должно быть сфорировано видение результата 

- рабочая группа 
- понимание пользователя 
- формирование метрик 
- договорённости о сроках 
- фиксация целей проекта 

![](../_png/Pasted%20image%2020260623232825.png)

##### Зачем нужен зафиксированный Scope в рамках ITIL4
...
> Когда есть зафиксированный Scope, мы можем гарантировать сроки и качество продукта 

>[!example] Если бизнес нас просит докинуть ещё функционала, то мы должны поставить их перед чётким фактом: у нас есть время, деньги и scope - новая фича отъест денег, накинет времени к разработке или урежет scope, чтобы вместиться 
...
Часто от нас ждут решение на основе опыта: 
...
>[!important] Но у команды опыт-то свой!
> Привлекая опыт команды, мы сокращаем область абсолютного незнания 

>[!warning] "Мы же договорились!" 
> Самая опасная фраза в управлении. Если мы договорились, то нужно письменно зафиксировать. 
...
>[!important] Намерения -> Слова -> Интерпретация -> Действие 
> При переходе к словам, мы теряем часть намерений. При переходе к интерпретации, мы теряем часть слов. И так далее.  

#### Типичные ошибки менеджера

- Слишком высокий уровень абстракции ("сделайте хорошо") 
- Договорились устно - не зафиксировали письменно 
- Не проверили понимание: "Всё понятно?" -> "Да" - не работает (если не понятно, то нужно вернуться и проговорить, как для камушка) 
...
> Придя в казино можно столкнуться с тем, что за день у людей может быть средняя прибыль $500, но в этом утверждении будет множество но: кто-то ушёл с огромным минусом, а кто-то один выиграл джек-пот. 
...
> Если смотреть на самолёты, которые прилетели и где в них пули, то можно подумать, что эти части нужно укреплять, но это будет ошибкой. Нужно смотреть не на уцелевшие самолёты, а на те, которые были сбиты - там места повреждений более достоверные для сохранения целостности. 

![](../_png/Pasted%20image%2020260624210518.png)

>[!example] Получается, опыт обесценивается?!
> На самом деле нет. Групповой опыт даёт разные точки зрения на проблему. 
...
- Стохастическая - Есть информация о распределении вероятности (когда мы внедряем ERP, мы имеем информацию о вендорах и можем представлять уйдёт он или не уйдёт) 
- Поведенческая - Есть информация о влиянии поведения на результат (если увеличим количество людей в системе, то она начнёт тормозит) 
- Природная - Имеется информация только о возможных результатах (может ураган снесёт электрический столб и всё вырубится) 
- Априорная - Нет никакой информации (мы не имеем никакой информации о подобном инцинденте) 
...
>[!info] Нельзя ничего познать на 100%
...
>[!success] Это хороший вариант, если есть время и деньги 
...
Если в наш датацентр прилетит метеорит, то всё встанет на какое-то время. Вероятность безумно мала, но цена высока: `0,00000001% * 1_000_000_000 = 10 rv`. 

Если мы выбираем какого-нибудь вендора и находим крайне дешёвого, мы должны сразу учесть то, что компания может обанкротиться. Поэтому нужно уточнить её финансовые показатели, с кем они работают, над чем. 
...
> Данными нужно пользоваться беспристстрастно 
...
| Цена ошибки      | Стоимость сбора данных | Правильное действие |
| ---------------- | ---------------------- | ------------------- |
...
> В рамках выбора вендора ERP платформы: 
> - Сильная сторона - низкая стоимость 
> - Слабая сторона - высокий шанс со стороны банкротства компании 
> - Возможности - можно достаточно быстро внедрить и начать приносить пользу 
> - Угрозы - вендор может уйти с рынка из-за санкций 

![](../_png/Pasted%20image%2020260624214340.png)

Поэтому нужно оценивать окружающую среду, чтобы оценить SWOT матрицу 
...
Так же обязательно нужно все факторы сопоставлять в моменте обсуждения. 

> Не смотря на то, что вендор может уйти с русского рынка, мы имеем зарубежное юрлицо, которое позволит продолжать общаться с ним. 
...
- Делают в одиночку, не привлекая тех, кто ближе к проблеме

##### Кейс

|                                      |                                                                                                                                                                     |                                                         |
| ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------- |
| S - ERP известна на локальном рынке  | SW - несмотря на то, что стоимость высокая, то, что она более популярная, позволяет проще адаптировать людей и нанимать более профильную команду                    | W - Высокая стоимость лицензии                          |
| O - Можно нанимать команду в будущем | OT - решение может не подойти для иностранных партнёров, но команда с опытом решения проблем может закрыть дыру за счёт адаптации и позволит снизить уровень угрозы | T - Решение может не подойти для иностранных партнеров  |
...
- Complex
	...
	- Пример: Обычно, это MVP. Мы приходим с гипотезой и проверяем её. 
- Chaotic
	- Хаотичное
	- Связи неопределены
	- Novel Practice -> Действуй -> Воспринимай -> Реагируй
	- Пример: Опыта решений проблем из этой области не существует, это пожар, поэтому нужно просто брать гипотезу и проверять  
- Confused
	- Disorder
...
5. Нововведения часто воспринимаются жёстко ("сейчас-то всё работает") - нужно подсветить минусы текущего решения 
...
>[!failure] Важнее != Исключает 
> Многие менеджеры откидывают вторую часть, но это будет некорректный подход. Процесс, документация и план - нужны, но они просто отступают на второй план на момент того, когда нам нужно сильно вложиться в продукт. 

Из ценностей следуют [12 принципов Agile](./agile.md#Agile-манифест) 
...
> - Практики, которые реализуются фреймворкамами, методами и подходами - Scrum, Kanban, XP, SAFe…

>[!cite] "Agile is not methodology. Agile is a mindset." - Ron Jeffries 
...
>[!example] NASA до сих пор использует Waterfall в своих проектах 
> В 1970 году запустили Voyager, который до сих пор отправляет нам сигнал и уже находится вне солнечной системы 
...
- В домене Complex (комплексное, требования возникают в процессе) - он ломается структурно, а не из-за плохого исполнения. Все современные продукты требуют постоянной доработки и они всегда движутся вперёд. 
...
Именно потому, что авторы Agile понимали этот риск. Это один из первых фреймворков, которые поддерживали Agile принципы. 
...
Scrum - это лёгкий фреймворк, который помогает людям, командам и организациям создавать ценность через адаптивное решение комплексных проблем. 
...
1. Commitment- обязанность выполнить в срок 
2. Courage - смелость 
3. Focus - фокус на результате 
4. Openness - открытость 
5. Respect - взаимоуважение 
...
> Команды, которые пропускают ценности и берут только артефакты - получают тот самый Cargo Cult и сталкиваются с непониманием того, зачем вообще нужен Scrum 
...
>[!hint] Оптимальный размер: 3-9 человек
...
>[!warning] Значит, ответственности тимлида никуда не делись - они просто не распределены 

>[!important] У Тимлида работы гораздо больше 

#### Scrum - Это SDLC фреймворк

Чего не рассматривает Scrum: 
...
- Product Goal: Прирастить конверсию A на Х% -> 
- Sprint Goal: Дать возможность пользователю размещать заказы нового типа на Desktop -> 
- Increment: Ушел в релиз и запущен А/Б тест с новыми типами заказов
...
2. Sprint Planning 
   ...
   ≤ 8 часов (нужно стараться укладываться в 2 часа) 
3. Daily Scrum. 
...
5. Sprint Retrospective 
   Инспекция команды + план улучшений (что хорошо, что плохо, что стоит улучшить) 
   ≤ 3 часа 
...
- "PO" - это должность, которую совмещает занятой руководитель (он должен быть выделен на продукт и погружен) 
...
>[!warning] Scrum Guide говорит, что он решает проблемы тогда и только тогда, когда все части фреймворка применяются 
...
> Интеграция показывает, что изменения не испортили ничего. Без автотестов - это Continuous Build. 
...
- и почему задачи перемещаются между колонками 
...
>[!book] Цель - процесс непрерывного улучшения (Элияху Голдратт) 
> Если на вход мы берём много задач, а на выходе выпускаем очень мало задач, то мы принесли мало денег компании. 
> Поэтому нужно набирать максимальное количество задач, которое может за единицу времени выезжают в production. 
...
> Например, мы поставили задачи разработке и передали скопом кучу вещей в тестирование. Потом эти задачи пропали надолго - они никуда не едут. В таком случае, узкое место - это тестирование. 

> Важно не только декомпозировать задачи, а ещё и следить за узкими местами в SDLC. 
...
> Нужен для описания образа большого результата, то есть описать цель 
...
> - Флаг: "Система" - что-то большое и непонятное (что уже точно требует Epic) 
...
В итоге у нас есть глобальный эпик, который мы делим на стори, в рамках которого пользователи выполняют определённое действие. По сторе мы почкуем отдельные задачи, которые принесут пользователю ценность в рамках данной стори. 

- Эпик - это глобальная цель 
- Сторя - это отдельная единица бизнес-ценности для пользователя 
- Таски - это ступеньки для решения стори 
...
- Горизонтально - закрываем сразу все кейсы в рамках каждого слоя по задаче 
	- Сначала БД 
	- Потом API 
	- Потом UI 
- Вертикально - закрываем бизнес-потребность 
	- Добавить товар 
	- Удалить товар 
	- Изменить количество 
	- Применить промокод 
...
- явный timebox (например, 4 часа) - на поиск возможного решения выделяем определённое время 
- явный output (решение / документ / ADR) - должны зафиксировать некоторые решения после этого поиска 
...
- Подзадача закрывается за конкретное время, например, 1-2 дня (максимум - 3-5) 
...
> На ретро стоит потом поднять вопрос, почему определённая задача выполнялась не 2 определённых дня, а 4 
...
>[!success] Обязательно нужно описать задачу по SMART 
...
> Это хороший инструмент, чтобы получать от бизнеса ясный кейс, а на выходе формировать то, что нужно стейкхолдеру 
...
>[!important] Сторя должна покрывать один флоу 
...
| S     | Small - маленькая (чтобы влезть в инкремент) |
...
- Фундамент:
	- Контекст (нужно объяснить тимлиду, а он потом разработчикам) 
...
- Границы и проверка
	- Что НЕ нужно делать (out of scope)
		- Явные ограничения
		- Что специально выносим за рамки (например, реализуем только уведомления по почте и пушам, а СМС выносим в отдельный скоуп и не делаем сейчас) 
	- Acceptance Criteria
...
- event storming 
- task kickoff (можно собраться со всеми и объяснить, как эта задача будет выглядеть в целом) 
...
>[!success] Лечение: Явный порядок с учётом зависимостей
...
>[!success] Лечение: WIP-лимиты, командная ответственность за спринт (а не за персональные задачи)
> Самый быстрый коллега не должен идти станчала в бэклог, а его силы должны быть направлены на то, чтобы помочь принести ценность, то есть помочь протолкнуть забуксованные задачи 
...
>[!warning] То есть тимлиду нужно декомпозировать задачи, когда: 
> - команда джунов 
> - когда сильно нужно затащить быстро фичу 
...
> Event Storming вытаскивает эти сценарии CJM на поверхность до того, как они стали багами в проде 
...
На выходе получается примерно такая карта: 

![](../_png/Pasted%20image%2020260918212356.png)
...
> Это защита команды от работы вслепую
...
> Они применяются для всех задач 
...
- Checklist
	...
	- Быстро и понятно (если никогда с AC не работали)
...
![](../_png/Pasted%20image%2020260919144801.png)
...
- Позволяет приоритизировать "Критический путь" и какие задачи будут блокировать друг друга 
...
>[!warning] Задачи критического пути нужно двигать выше по приоритету! 
...
>[!example] Нам нужно провести рефакторинг приложения, чтобы оно работало быстрее. 
...
>[!example] Если к нам придут и скажут, что "мы делаем мало PR", то: 
...
> И как укладываться в 4 минуты downtime в месяц? 
> Тут уже инжинеры подходят к вопросам эскалиривания, резервирования, рекавери, на которые отвечает System Design. 
...
>[!info] Эту работу эффективно оптимизирует ИИ
...
>[!warning] Проблема Alert Fatigue:
...
>[!failure] Если алертов слишком много - они перестают работать
...
> Каждый алерт должен требовать немедленного человеческого действия. 
> Если алерт можно проигнорировать -> его не должно быть
...
### Когда команду просят ускориться 

А ускориться относительно чего? Нужно явно задать метрику, по которой считают скорость команды и куда мы должны стремиться. 

>[!tip] Нам кажется, что самолёт в небе медленно движется, но на самом деле он ишачит со скоростью 1000км/ч 
> Тут решает метрика, по которой мы считаем скорость. 

>[!success] Сначала нужно посчитать TTM, а уже потом считать, где мы тормозим и как можно ускоряться. 

### Когда проходит много задач 

Тимлид не должен лезть в код каждого крупного решения, которое ему приносит команда. Мы смотрим на архитектуру и даём аппрув на разработку. Ответственность за результат так же должна будет оставаться на сотруднике. 
