...
`this` — всегда ссылка на текущий объект. 
Можно создавать сколько угодно экземпляров одного класса.
...
Если сеттер отсутствует - свойство readonly. Если геттер отсутствует - получить значение невозможно и оно writeonly.
...
> [!WARNING] Наследование может быть вредно в реальных проектах. При большом числе наследников изменение родительского класса ломает всех потомков - это называется **проблемой хрупкого базового класса**. Глубокая иерархия (3+ уровней) становится трудно понимаемой и сопровождаемой. Стоит редпочитать **композицию** наследованию.

#### Полиморфизм

**Полиморфизм** - «один интерфейс — множество реализаций» - это механизм, который позволяет работать с разными типами через единый интерфейс. 
...
##### Полиморфизм подтипов (переопределение)
