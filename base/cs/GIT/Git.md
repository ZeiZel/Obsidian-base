...
Есть много различных интуитивных подходов для контроля версий файлов:
...
- сбора релиза, которые используют определённый тег.
...
И все инструменты говорят нам, что мы находимся в режиме отцеплённой головы
...
Сами по себе пустые папки иногда нужно сохранить, когда они нужны для других механизмов приложения, но в режиме разработки в ней ничего нет. Обычно такие папки добавляют, когда какие-то инструменты ломаются при отсутствии папки (не предусмотрена проверка наличия и создания папки).
...
Так же мы можем восстановить целиковую удалённую ветку. Восстановить её нам удастся, если мы перейдём на её последний коммит, попадём в detached HEAD и создадим от него новую ветку. 
...
- Возможность разрабатывать совместно в команде с другими разработчиками
...
Worktree - это инструмент для создания нескольких рабочих деревьев

Например, если нам нужно переключиться на hotfix не прерывая текущую работу, без `stash`. 

Каждое дерево - отдельная папка с общим `.git`.
...
Механизм filter-repo - это переписывание истории. Мы можем стереть те данные, которые мы не хотели публиковать, но по ошибке отправили их
...
>[!info] Лучше использовать `git filter-repo` вместо устаревшего `git filter-branch` — быстрее и безопаснее.
...
Позволяет выполнить операции над определённой выборкой коммитов. Например, он найдёт за O(log n) проверок коммит, который сломал функциональность: ставим метку на сломанном коммите и на последнем рабочем и дальше пойдёт проверка.
